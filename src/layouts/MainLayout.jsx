...
function MainLayout() {
  const { socketConnected, notifications, activityFeed, unmappedProducts, products } = useStore();
  ...
  const handleSimulateOrder = async () => {
    ...
    try {
      ...
      const orderRes = await axios.post(
        `http://localhost:4000/api/${simMarketplace.toLowerCase()}/orders`,
        {
  ...
  const getSimMarketplaceProducts = () => {
    ...
    if (simMarketplace === 'shopee') {
      return [
        { id: 'SHP-ELEC-001-ID', sku: 'SHP-ELEC-001', name: 'Wireless Bluetooth Earbuds Pro (SHP-ELEC-001)' },
        { id: 'SHP-FASH-002-ID', sku: 'SHP-FASH-002', name: 'Slim Fit Cotton Polo Shirt (SHP-FASH-002)' },
        { id: 'SHP-HOME-003-ID', sku: 'SHP-HOME-003', name: 'Ceramic Non-Stick Frying Pan (SHP-HOME-003)' },
        { id: 'SHP-SPORT-004-ID', sku: 'SHP-SPORT-004', name: '[UNMAPPED] Running Shoes Lightweight (SHP-SPORT-004)' },
        { id: 'SHP-BEAUTY-005-ID', sku: 'SHP-BEAUTY-005', name: '[UNMAPPED] Vitamin C Serum 30ml (SHP-BEAUTY-005)' }
      ];
    } else if (simMarketplace === 'tokopedia') {
      return [
        { id: 'TOK-ELEC-001-ID', sku: 'TOK-ELEC-001', name: 'Mechanical Gaming Keyboard RGB (TOK-ELEC-001)' },
        { id: 'TOK-BOOK-005-ID', sku: 'TOK-BOOK-005', name: 'Clean Code Handbook (TOK-BOOK-005)' },
        { id: 'TOK-FOOD-003-ID', sku: 'TOK-FOOD-003', name: 'Premium Arabica Coffee Beans (TOK-FOOD-003)' },
        { id: 'TOK-FURN-002-ID', sku: 'TOK-FURN-002', name: '[UNMAPPED] Ergonomic Office Chair (TOK-FURN-002)' },
        { id: 'TOK-AUTO-004-ID', sku: 'TOK-AUTO-004', name: '[UNMAPPED] Car Dash Camera (TOK-AUTO-004)' }
      ];
    } else {
      return [
        { id: 'LZD-ELEC-001-ID', sku: 'LZD-ELEC-001', name: 'Smart Watch Fitness AMOLED (LZD-ELEC-001)' },
        { id: 'LZD-HOME-003-ID', sku: 'LZD-HOME-003', name: 'Robot Vacuum Cleaner LiDAR (LZD-HOME-003)' },
        { id: 'LZD-FASH-002-ID', sku: 'LZD-FASH-002', name: 'Genuine Leather Wallet RFID (LZD-FASH-002)' },
        { id: 'LZD-BABY-004-ID', sku: 'LZD-BABY-004', name: '[UNMAPPED] Baby Stroller Lightweight (LZD-BABY-004)' },
        { id: 'LZD-ELEC-006-ID', sku: 'LZD-ELEC-006', name: '[UNMAPPED] USB-C Hub 10-in-1 (LZD-ELEC-006)' }
      ];
    }
  };
  ...
  React.useEffect(() => {
    if (simulatorOpen) {
      axios.get(`http://localhost:4000/api/${simMarketplace.toLowerCase()}/products`)
        .then(res => {
          const list = res.data.data.products || res.data.data || [];
          setMockProducts(list);
          if (list.length > 0) setSimProduct(list[0].item_id || list[0].product_id || list[0].id);
        })
        .catch(err => console.error('Gagal mengambil produk mock untuk simulator', err));
    }
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-xs">
              {socketConnected ? (
                <>
                  ...
                  <span className="text-slate-300 font-medium">Realtime: Connected</span>
                </>
              ) : (
                <>
                  <WifiOff size={14} className="text-red-400" />
                  <span className="text-slate-400 font-medium font-mono">Realtime: Offline</span>
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-slate-800">
              ...
              <span className="text-sm font-medium text-slate-300 hidden md:block">Admin CTO</span>
            </div>
          </div>
        </header>
          ...
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-slate-950">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-white text-base">E-Commerce Webhook Simulator</span>
              </div>
              <button 
                onClick={() => {
            ...
            <div className="p-5 space-y-4">
              <p className="text-xs text-slate-400 leading-relaxed">
                Gunakan panel ini untuk mensimulasikan pembelian langsung di marketplace. Mock API akan membuat transaksi baru, memicu webhook, dan mengirimkan payload order ke aggregator.
              </p>

              {/* Marketplace Select */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase">Pilih Marketplace</label>
                <select
              ...
              {/* Product Select */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase">Pilih Produk</label>
                <select
                  value={simProduct}
                ...
                >
                  {mockProducts.length === 0 ? (
                    <option value="">Mengambil data dari marketplace mock...</option>
                  ) : (
                    mockProducts.map((p) => {
                      ...
                      return (
                        <option key={id} value={id}>
                          {name} ({sku}) - Stock: {p.stock !== undefined ? p.stock : p.available}
                        </option>
                      );
                    })
                  )}
              ...
              {/* Quantity Input */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-400 uppercase">Kuantitas Pembelian</label>
                <input
                  type="number"
                  min="1"
                  max="100"
              ...
              {simResult && (
                ...
                >
                  ...
                  <div className="flex-1 leading-normal font-medium">{simResult.message}</div>
                </div>
              )}
            </div>
