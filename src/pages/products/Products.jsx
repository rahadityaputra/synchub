...
function Products() {
  ...
  const handleCreateProduct = async () => {
    ...
    const internalSku = newProduct.internalSku.trim();
    const stock = Number.parseInt(newProduct.stock, 10);
    const price = Number.parseInt(newProduct.price, 10);

    if (!name || !internalSku || Number.isNaN(stock) || Number.isNaN(price)) {
      return;
    }
  ...
  const filteredProducts = products.filter((p) => {
    const productSku = p.sku || p.internalSku || "";
    const matchesSearch =
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-1">
                <span className="text-[10px] text-slate-500 font-semibold uppercase font-mono">
                  {editProduct.sku || editProduct.internalSku || "-"}
                </span>
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex gap-2.5 items-start text-[11px] text-slate-400">
                ...
                <p className="leading-normal">
                  Mengubah stok di gudang aggregator akan memicu antrean
                  sinkronisasi stok secara otomatis ke semua SKU marketplace
                  yang terhubung.
                </p>
              </div>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={newProduct.internalSku}
                    onChange={(e) =>
                      setNewProduct((prev) => ({
                        ...prev,
                        internalSku: e.target.value,
                      }))
                    }
                    placeholder="SKU-MOUSE-001"
              ...
              <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex gap-2.5 items-start text-[11px] text-slate-400">
                ...
                />
                <p className="leading-normal">
                  Request yang dikirim akan mengikuti format: name, internalSku,
                  stock, dan price. Setelah berhasil, produk akan langsung
                  muncul di daftar inventori.
                </p>
              </div>
              ...
              <button
                ...
                disabled={
                  ...
                  !newProduct.internalSku.trim() ||
                  newProduct.stock === "" ||
                  newProduct.price === ""
                }
                className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-lg shadow-lg text-xs transition-all flex items-center justify-center gap-2"
              >
                {creating ? (
                  <span className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
                ) : (
