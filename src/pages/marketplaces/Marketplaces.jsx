...
function Marketplaces() {
  ...
  useEffect(() => {
    marketplaces.forEach(async (m) => {
      try {
        const res = await api.get(`/${m.id}/status`);
        if (res.data?.data?.errorMode) {
          setSimMode((prev) => ({ ...prev, [m.id]: res.data.data.errorMode }));
        }
      } catch (err) {
        console.warn(
          `Gagal memuat status error mode untuk ${m.id}`,
          err.message,
        );
      }
    });
  ...
  const handleTestConnection = async (id) => {
    ...
    try {
      ...
      addNotification(
        "success",
        `Koneksi ke API ${id.toUpperCase()} berhasil.`,
      );
    } catch (err) {
      setTestResult((prev) => ({
        ...prev,
      ...
      addNotification(
        "error",
        `Koneksi ke API ${id.toUpperCase()} gagal: ${err.message}`,
      );
    } finally {
      setTestingId(null);
    }
  ...
  const handleApplySimulatedError = async (marketplace, mode) => {
    ...
    try {
      await api.post(`/${marketplace}/simulate-error`, { mode });
      setSimMode((prev) => ({ ...prev, [marketplace]: mode }));

      if (mode === "none") {
        addNotification(
          "success",
          `Mode error simulasi ${marketplace.toUpperCase()} dinonaktifkan (Normal).`,
        );
      } else {
        addNotification(
          "warning",
          `Simulasi error ${marketplace.toUpperCase()} diubah ke: ${mode.toUpperCase()}`,
        );
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400">
          Konfigurasi token API, webhook, status kanal, dan simulator kegagalan
          jaringan
        </p>
      </div>
            ...
            >
              <div>
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-800/80">
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-8 w-8 rounded-lg flex items-center justify-center font-bold text-white text-xs ${config.bgColor}`}
                    >
                      {m.name.charAt(0)}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] font-bold ${m.status === "ACTIVE" ? "text-emerald-400" : "text-slate-500"}`}
                    >
                      {m.status}
                    </span>
                    <button
                    ...
                    <div className="grid grid-cols-2 gap-1.5 text-[10px]">
                      {[
                        ...
                        <button
                          key={opt.value}
                          onClick={() =>
                            handleApplySimulatedError(m.id, opt.value)
                          }
              ...
              <div className="mt-5 pt-4 border-t border-slate-800/80 space-y-3 shrink-0">
                ...
                >
                  <RefreshCw
                    size={12}
                    className={testingId === m.id ? "animate-spin" : ""}
                  />
